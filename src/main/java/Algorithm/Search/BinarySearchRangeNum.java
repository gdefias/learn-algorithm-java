...
public class BinarySearchRangeNum {
    ...
    public static int Solution2(int[] A, int k) {
        ...
        int left = Algorithm.Search.BinarySearchFrist.binarySearch(A, k);
        if(left == -1) {
            return 0;
        }

        int right = Algorithm.Search.BinarySearchLast.binarySearch(A, k);
        return right-left+1;
    }


    //方法2：两次二分查找分别找相邻两元素的同一边界(target和target+1，即使target+1不在数组中也没关系)
